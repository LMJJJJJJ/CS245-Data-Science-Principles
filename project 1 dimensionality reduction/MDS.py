...
if __name__ == '__main__':
    parser = argparse.ArgumentParser("MDS decomposition")
    ...
    data_save_folder = f"./data/MDS/{name}"
    fig_save_folder = f"./fig/MDS"
    makedirs(data_save_folder)
    makedirs(fig_save_folder)

    mds = MDS(n_components=args.n_components, verbose=1)

    X_decomposed = mds.fit_transform(X)
