...
if __name__ == '__main__':
    ...
    parser.add_argument("--preprocess", type=int, default=64)
    args = parser.parse_args()
    X, y = load_raw()

    pca = PCA(n_components=args.preprocess)
    X = pca.fit_transform(X)
    print("Preprocess finished!")

    name = f"{args.n_components}_{args.init}_{args.method}_{args.preprocess}"
    data_save_folder = f"./data/PCA-tSNE/{name}"
    fig_save_folder = f"./fig/PCA-tSNE"
    makedirs(data_save_folder)
    makedirs(fig_save_folder)

    tSNE = TSNE(n_components=args.n_components, verbose=1, init=args.init, method=args.method, n_jobs=2)
