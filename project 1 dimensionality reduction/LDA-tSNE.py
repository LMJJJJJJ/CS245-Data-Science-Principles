...
if __name__ == '__main__':
    ...
    parser.add_argument("--preprocess", type=int, default=49)
    args = parser.parse_args()
    X, y = load_raw()

    lda = LinearDiscriminantAnalysis(n_components=args.preprocess)
    lda.fit(X, y)
    X = lda.transform(X)
    print("Preprocess finished!")

    name = f"{args.n_components}_{args.init}_{args.method}_{args.preprocess}"
    data_save_folder = f"./data/LDA-tSNE/{name}"
    fig_save_folder = f"./fig/LDA-tSNE"
