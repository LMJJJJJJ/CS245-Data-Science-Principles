...
class BDA(object):
    def __init__(self, kernel_type='primal', dim=30, lamb=1, mu=0.5, gamma=1, T=10, mode='BDA', estimate_mu=0):
        '''
        Init func
    ...
    def fit_predict(self, Xs, Ys, Xt, Yt):
        ...
        for t in range(self.T):
            ...
            if self.estimate_mu == 1 and self.mode == 'BDA':
                if Xs_new is not None:
                    mu = estimate_mu(Xs_new, Ys, Xt_new, Y_tar_pseudo)
                else:
                    mu = 0
            print("[ITER {}] mu: {}".format(t, mu))
            M = (1 - mu) * M0 + mu * N
